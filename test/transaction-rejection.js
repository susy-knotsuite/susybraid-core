...
describe("Transaction rejection", function() {
  ...
  before("lock account 1", async function() {
    ...
    await susyweb.sof.personal.lockAccount(accounts[1]);
  });
  ...
  it("should reject transaction if insufficient funds", async function() {
    ...
    await testTransactionForRejection(
      {
        value: susyweb.utils.toWei("100000", "sophy")
      },
  ...
  async function testTransactionForRejection(paramsOverride, expectedMessage) {
    ...
    // don't send with susyweb because it'll inject its own checks
    const response = await send("sof_sendTransaction", params).catch((e) => ({ error: e }));
    ...
    if (response.error) {
      ...
      const receipt = await susyweb.sof.getTransactionReceipt(response.result);
      if (!receipt.status) {
      ...
      assert.fail(new Error("sof_sendTransaction responded with empty RPC response"));
    }
  }
});
