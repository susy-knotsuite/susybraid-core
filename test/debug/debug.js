...
function test(forked) {
  ...
  */
  ...
  before("set up susyweb and contract", async function() {
    this.timeout(10000);
    const contractRef = {
  ...
  describe("Trace a successful transaction", function() {
    ...
    it("should trace a successful transaction without changing state", async function() {
      ...
      // To at least assert SOMSOFING, let's assert the last opcode
      assert(structLogs.length > 0);

      for (let op of structLogs) {
        if (op.stack.length > 0) {
          // check formatting of stack - it was broken when updating to sophonjs-vm v2.3.3
