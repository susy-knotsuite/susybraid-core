...
const confirmGasPrice = async(
  ...
  const _transferAmount = susyweb.utils.toWei(numberToBN(transferAmount), "finney");
  ...
  const receipt = await susyweb.sof.sendTransaction(params);

  // Convert gasUsed to a big number
  const gasUsed = numberToBN(receipt.gasUsed);

  // Get balance of sender account
  const finalBalance = await susyweb.sof.getBalance(accounts[0]);
  ...
  assert(
    ...
    `Gas price used by SVM (${to.hex(actualGasPriceBN)}) was different from` +
