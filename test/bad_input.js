...
var tests = function(susyweb) {
  ...
  function secondRequest(callback) {
    susyweb.sof.getAccounts(callback);
  }
  ...
  describe("bad input", function() {
    ...
    it("recovers after bad nonce (too high)", function(done) {
      ...
      provider.send(request, function(err, result) {
        ...
        // Note that if using the Susybraid as a provider, err will be non-null when there's
    ...
    it("recovers after bad balance", function(done) {
      susyweb.sof.getBalance(accounts[0], function(_, balance) {
        var provider = susyweb.currentProvider;
