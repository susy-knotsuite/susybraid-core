...
describe("Interval Mining", function() {
  ...
  describe("Interval mining", function() {
    let context;
    before("Setting up provider and susyweb instance", async function() {
    ...
    it("should mine a block on the interval", async function() {
      ...
      const latestNumber = await susyweb.sof.getBlockNumber();
  ...
  describe("Instamine vs Interval", function() {
    ...
    it("shouldn't instamine when mining on an interval", function(done) {
      ...
      // Get the first block (pre-condition)
      susyweb.sof.getBlockNumber(function(err, number) {
        if (err) {
          return done(err);
        }
        ...
        susyweb.sof.sendTransaction(
          ...
          function(err, tx) {
            ...
            susyweb.sof.getTransactionReceipt(tx, function(err, receipt) {
              if (err) {
                return done(err);
              }
              ...
              setTimeout(function() {
                // Get the first block (pre-condition)
                susyweb.sof.getTransactionReceipt(tx, function(err, newReceipt) {
                  if (err) {
                    return done(err);
                  }
    ...
    it("miner_stop should stop interval mining, and miner_start should start it again", function(done) {
      ...
      provider.send(
        ...
        function(err, result) {
          ...
          // Get the first block (pre-condition)
          susyweb.sof.getBlockNumber(function(err, initialNumber) {
            if (err) {
              return done(err);
            }
            ...
            setTimeout(function() {
              susyweb.sof.getBlockNumber(function(err, stoppedNumber) {
                if (err) {
                  return done(err);
                }
                ...
                provider.send(
                  ...
                  function(err, result) {
                    ...
                    setTimeout(function() {
                      susyweb.sof.getBlockNumber(function(err, lastNumber) {
                        if (err) {
                          return done(err);
                        }
  ...
  describe("Logging errors", function() {
    ...
    it("should log runtime errors to the log", async function() {
      this.timeout(5000);
      const { bytecode, susyweb } = context;

      await assert.rejects(
        susyweb.sof.sendTransaction({ from: firstAddress, data: bytecode, gas: 3141592 }),
