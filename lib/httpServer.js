...
function createCORSResponseHeaders(method, requestHeaders) {
  ...
  if (isCORSRequest) {
    ...
    // is set. Because susyweb@1.0.0-beta.35-? always sets `request.withCredentials = true` while Safari requires it be
    ...
    //  Regression Commit: https://octonion.institute/susy-js/susyweb.js/pull/1722
    //  Open SusyWeb Issue: https://octonion.institute/susy-js/susyweb.js/issues/1802
...
module.exports = function(provider, logger) {
      ...
      .on("end", function() {
        ...
        switch (method) {
          case "POST":
            ...
            if (payload.method === "sof_subscribe" || payload.method === "sof_unsubscribe") {
  ...
  server.susybraidProvider = provider;
  return server;
};
