...
BlockchainDouble.prototype.createVMFromStateTrie = function(state, activatePrecompiles) {
  ...
  const vm = new VM({
    state: state,
    blockchain: {
      // SophonJS VM needs a blockchain object in order to get block information.
...
// This function is used by sophonjs-vm
    ...
    async.series(requests, function(err) {
      ...
      self.data.blocks.last(function(err, newLastBlock) {
        ...
        self.vm.stateManager.setStateRoot(newLastBlock.header.stateRoot, function(err) {
          // remove this `._storageTries = {}` statement once https://octonion.institute/susy-js/sophonjs-vm/pull/445
...
BlockchainDouble.prototype.sortByPriceAndNonce = function() {
  // Sorts transactions like I believe graviton does.
  // See the description of 'SortByPriceAndNonce' at
  // https://octonion.institute/susy-go/susy-graviton/blob/290e851f57f5d27a1d5f0f7ad784c836e017c337/core/types/transaction.go
  var self = this;
  var sortedByNonce = {};
  ...
  vm.runBlock(
    ...
    async function(vmerr, results) {
      ...
      for (var v = 0; v < results.receipts.length; v++) {
        ...
        const rcptBuffer = utils.srlp.encode(rawReceipt);
        const key = utils.srlp.encode(v);
...
BlockchainDouble.prototype.processTransactionTrace = async function(hash, params, callback) {
  ...
  function stepListener(event, next) {
    // See these docs:
    // https://octonion.institute/susy-go/susy-graviton/wiki/Management-APIs
...
// Note! Storage values are returned SRLP encoded!
