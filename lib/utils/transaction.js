const SophonJsTransaction = require("sophonjs-tx");
const SophonJsFakeTransaction = require("sophonjs-tx/fake");
const sofUtil = require("sophonjs-util");
const assert = require("assert");
const srlp = require("srlp");
const to = require("./to");

const sign = SophonJsTransaction.prototype.sign;
const fakeHash = function() {
  // this isn't memoization of the hash. previous versions of susybraid-core
  ...
  return SophonJsFakeTransaction.prototype.hash.apply(this, arguments);
};
...
/**
 * sophyuemjs-tx's Transactions don't behave quite like we need them to, so
...
function fixProps(tx, data) {
  // sophonjs-tx doesn't allow for a `0` value in fields, but we want it to
  // in order to differentiate between a value that isn't set and a value
  // that is set to 0 in a fake transaction.
  // Once https://octonion.institute/susy-js/sophonjs-tx/issues/112 is figured
  ...
  // Sophonjs-tx doesn't set the _chainId value whenever the v value is set,
  // which causes transaction signing to fail on transactions that include a
  // chain id in the v value (like sophys.js does).
  ...
  Object.defineProperty(tx, "v", {
    set: (v) => {
      ...
      const sigV = sofUtil.bufferToInt(tx.v);
...
function initData(tx, data) {
  if (data) {
    ...
    if (Array.isArray(data)) {
      ...
      data.forEach((d, i) => {
        self[self._fields[i]] = sofUtil.toBuffer(d);
      });
...
module.exports = class Transaction extends SophonJsTransaction {
  /**
  ...
  static fromJSON(json, type) {
    ...
    if (json.to) {
      ...
      if (buf.equals(Buffer.from([0]))) {
        ...
        toAccount = sofUtil.setLengthLeft(buf, 20);
      } else {
        toAccount = buf;
      }
    }
    ...
    const options = {
      nonce: sofUtil.toBuffer(to.hex(json.nonce)),
      from: sofUtil.toBuffer(to.hex(json.from)),
      value: sofUtil.toBuffer(to.hex(json.value)),
      gasLimit: sofUtil.toBuffer(to.hex(json.gas || json.gasLimit)),
      gasPrice: sofUtil.toBuffer(to.hex(json.gasPrice)),
      data: data ? to.buffer(data) : null,
      to: toAccount,
      v: sofUtil.toBuffer(json.v),
      r: sofUtil.toBuffer(json.r),
      s: sofUtil.toBuffer(json.s)
    };
   ...
   *
   * This method is nearly identical to sophonjs-tx hash with the exception of
