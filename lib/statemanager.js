...
var wallet = require("sophonjs-wallet");
...
var sigUtil = require("sof-sig-util");
...
StateManager.prototype._applyDefaultOptions = function(options) {
  ...
  // our settings back to our consumer application (e.g., susybraid)
...
StateManager.prototype.initialize = function(callback) {
  ...
  let defaultBalanceWei = to.hex(SusyWeb.utils.toWei(self.options.default_balance_sophy.toString(), "sophy"));
...
StateManager.prototype.mineOnInterval = function() {
  ...
  // make sure a pending sof_mine doesn't come back and execute mineOnInterval
  // again...
...
StateManager.prototype.queueRawTransaction = function(data, callback) {
  ...
  this._queueTransaction("sof_sendRawTransaction", tx, from, null, callback);
};
...
StateManager.prototype.queueTransaction = function(method, txJsonRpc, blockNumber, callback) {
  ...
  if (method === "sof_sendTransaction" && !this.unlocked_accounts.hasOwnProperty(from)) {
  ...
  if (!isKnownAccount || method === "sof_call") {
...
StateManager.prototype.processNextAction = function(override) {
  ...
  if (typeof queued.method === "function") {
  ...
  } else if (queued.method === "sof_getStorageAt") {
    this.blockchain.getStorage(queued.address, queued.position, queued.block, function(err, result) {
      ...
      if (result) {
        result = utils.srlp.decode(result);
      }
  ...
  } else if (queued.method === "sof_sendTransaction" || queued.method === "sof_sendRawTransaction") {
    this.processTransaction(queued.from, queued.tx, intermediary);
  } else if (queued.method === "sof_call") {
    this.processCall(queued.from, queued.tx, queued.blockNumber, intermediary);
  } else if (queued.method === "sof_estimateGas") {
...
StateManager.prototype.signTypedData = function(address, typedDataToSign) {
  ...
  if (!typedDataToSign.types.SIP712Domain) {
    throw new Error("cannot sign data; SIP712Domain definition missing");
  }
...
StateManager.prototype.getLogs = function(filter, callback) {
  ...
  // https://octonion.institute/susy-go/wiki/JSON-RPC#sof_getlogs
