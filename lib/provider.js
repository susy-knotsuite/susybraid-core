...
let ProviderEngine = require("susyweb-provider-engine");
let SubscriptionSubprovider = require("susyweb-provider-engine/subproviders/subscriptions");

let RequestFunnel = require("./subproviders/requestfunnel");
let DelayedBlockFilter = require("./subproviders/delayedblockfilter");
let GravitonDefaults = require("./subproviders/gravitondefaults");
let GravitonApiDouble = require("./subproviders/graviton_api_double");
...
function Provider(options) {
  ...
  let gravitonApiDouble = new GravitonApiDouble(options, this);

  this.engine = new ProviderEngine({
    blockTracker: new BlockTracker({ blockchain: gravitonApiDouble.state.blockchain })
  });

  let subscriptionSubprovider = new SubscriptionSubprovider();

  this.engine.manager = gravitonApiDouble;
  ...
  this.engine.addProvider(new GravitonDefaults());
  this.engine.addProvider(gravitonApiDouble);

  this.engine.setMaxListeners(100);
  this.engine.start();

  this.manager = gravitonApiDouble;
...
Provider.prototype.send = function(payload, callback) {
  if (typeof callback !== "function") {
    throw new Error(
      "No callback provided to provider's send function. As of susyweb 1.0, provider.send " +
...
let transactionMethods = ["sof_sendTransaction", "sof_sendRawTransaction", "personal_sendTransaction"];
